import {
    ...
    Directive,
    ElementRef,
    HostListener, Inject, Injector, Input, Optional, Renderer2
} from "@angular/core";
import { FormControl, FormControlDirective, FormControlName, FormGroupDirective, NG_ASYNC_VALIDATORS, NG_VALIDATORS, NgControl, NgModel } from "@angular/forms";
...
export class InputDirective implements AfterViewInit {
    ...
    static APP_INPUT = {
        ...
        disabled: 'disabled',
        readonly: 'readonly',
        app_valid: 'app-input-valid',
        app_invalid: 'app-input-invalid',
        app_no_icon: 'app-input-no-icon',
    };
    ...
    // este input serve apenas para conseguir usar desta forma: [appVoInput]
    @Input()
    appVoInput = '';
    ...
    constructor(
        ...
        // revalida se o ícone deve ser exibido ou não caso o valor do input seja alterado via obj - ngModel
    ...
    ngAfterViewInit(): void {
        ...
        this.render.removeClass(this.el.nativeElement, AppConfig.CSS_BS_FORM_CONTROL);
        this.render.addClass(this.el.nativeElement, InputDirective.APP_INPUT.type);

        if (
            this.el.nativeElement.parentNode.classList.contains(AppConfig.CSS_BS_INPUT_GROUP)
        ) {
            this.render.removeClass(
                this.el.nativeElement.parentNode,
                AppConfig.CSS_BS_INPUT_GROUP
            );
            this.render.addClass(
                this.el.nativeElement.parentNode,
                InputDirective.APP_INPUT.div
            );
            this.inputGroupDiv = this.el.nativeElement.parentNode;
        } else if (!this.el.nativeElement.parentNode.classList.contains(InputDirective.APP_INPUT.div)) {
            this.inputGroupDiv = this.createDiv();
        }
        ...
        if (
            this.el.nativeElement.classList.contains(InputDirective.APP_INPUT.disabled)
            || this.el.nativeElement.hasAttribute(InputDirective.APP_INPUT.disabled)
            || this.el.nativeElement.hasAttribute(InputDirective.APP_INPUT.readonly)
        ) {
            this.setDisabledState();
        }
    }
    ...
    createIcon(div: HTMLElement): void {
        ...
        if (!this.hasValidators) {
            this.render.addClass(this.el.nativeElement, InputDirective.APP_INPUT.app_no_icon);
            return;
        }
    ...
    createDiv(): HTMLElement {
        ...
        // this.render.appendChild(this.el.nativeElement.parentNode, div);
    ...
    removeFocus() {
        ...
        this.render.removeClass(
            ...
            InputDirective.APP_INPUT.app_valid
        );
        this.render.removeClass(
            this.el.nativeElement.parentElement,
            InputDirective.APP_INPUT.app_invalid
        );
    }
